...
EasyApi is an IntelliJ IDEA plugin (v3.0 rewrite) for API development — export API documentation to YApi/Postman/Markdown, send requests, and manage endpoints directly from source code.
...
- **Bug reports use the GitHub issue templates in `.github/ISSUE_TEMPLATE/`** (`bug_report.md`). Read the submitted fields first — they identify the trigger action, framework(s), custom-rule setup, and repro steps before you open the code. Question and feature requests have their own templates in the same folder.
- **PRs follow the Architecture & Threading and Logging checklists in this document.** When a checklist item does not apply to a change, state why in the PR description rather than deleting it.
