...
under `core/ai/`, `src/main/resources/ai/`, or `skills/easy-api-assistant/`.
...
| 5 | **Surfaces** | The two things people actually touch: the chat panel and the external skill | `core/ai/ui/`, `skills/easy-api-assistant/` | [§6](#6-chat-ui), [§7](#7-external-skill) |
...
skills/easy-api-assistant/               # external skill: mirrors, CLI, generated catalogs
...
[`skills/easy-api-assistant/tools.md`](../../skills/easy-api-assistant/tools.md):
...
Bundled scripts under `skills/easy-api-assistant/scripts/` mirror some of those
...
capabilities — [`SKILL.md`](../../skills/easy-api-assistant/SKILL.md) documents that
emulation per tool.
...
| Detection recipes / key guides | `ai/detection/`, `ai/key-guides/` | B | → skill `ai/` | `syncAgentCatalog` (auto) | `EasyApiAssistantSkillTest`, `KeyGuideSharedSectionParityTest`, `KeyGuideKeyResolvesTest`, `KeyGuideSchemeStampTest` |
| Magic templates | `ai/magic/` | B | not mirrored | — | `MagicInstructionBuilderTest` |
| Knowledge base | `docs/knowledge-base/` | B | → plugin resource + skill `docs/` | `syncKnowledgeBase` (auto) | `EasyApiAssistantSkillTest` |
| Rule-key catalog | `skills/…/rule-keys.{json,md}` | A | ← `*RuleKeys` objects + implicit keys | `syncRuleKeySchemes` (manual) | `RuleKeySchemeExporterTest`, `EasyApiAssistantSkillTest` |
| Script-context catalog | `skills/…/rule-contexts.{json,md}` | A | ← script-object signatures | `syncRuleContexts` (manual) | `EasyApiAssistantSkillTest` |
| Tool / locale / EP fact sheets | `skills/…/{tools,locales,extensions}.md` | A | ← `RuleTools` registries, `BundledLanguageTemplates`, `plugin.xml` | `syncSkillFacts` (manual) | `EasyApiAssistantSkillTest` |
...
Install with `npx skills add tangcent/easy-api -g -y`. The skill carries the
