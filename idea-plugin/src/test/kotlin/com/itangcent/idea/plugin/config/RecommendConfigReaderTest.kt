...
internal abstract class RecommendConfigReaderTest : AdvancedContextTest() {
    ...
    internal class SimpleRecommendConfigReaderTest : RecommendConfigReaderTest() {
        ...
        fun test() {
            ...
            run {
                var configs = ""
                recommendConfigReader.foreach({ it.startsWith("field.") }) { key, value ->
                    if (configs.isNotEmpty()) {
                        configs = "$configs\n"
                    }
                    configs += "$key=$value"
                }
                assertEquals(ResultLoader.load("foreach.field"), configs)
            }
            assertEquals(listOf("#ignore", "@Ignore").toJson(), recommendConfigReader.read("ignore").toJson())
            assertEquals("#module", recommendConfigReader.resolveProperty("\${module}"))
        }
    }
