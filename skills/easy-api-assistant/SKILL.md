...
(Postman/Markdown/cURL/Hoppscotch), field naming, annotations, and more.
...
works after `npx skills add tangcent/easy-api -g -y` even when the easy-api
...
# Check if a key is already configured (avoid duplicates — Quality Rule 1)
scripts/get_existing_rules_for_key.sh field.name method.doc
```
...
- Custom security annotations (e.g. `@RequirePermission`) that should become
  an `method.doc` or a Postman header.
