...
generated snapshot in `skills/easy-api-assistant/rule-keys.md`
...
- **Execution mode and bound objects** — `get_rule_context(key="<key>")` renders
  ...
  generated catalog — `skills/easy-api-assistant/rule-contexts.md` (every
...
- **Aliases** — legacy names accepted in place of the canonical key. Author
  ...
  `skills/easy-api-assistant/rule-keys.md`, or the `aliases` field of
...
`skills/easy-api-assistant/locales.md` carries the current set
...
| *(none)* | LiteralParser / TypeMatchParser | A literal value, or a type name matched against the element's type | `method.doc=disabled` |
...
```
method.doc[$class:com.example.UserCtrl]=user
method.doc[$class:com.example.UserCtrl]=admin
```
