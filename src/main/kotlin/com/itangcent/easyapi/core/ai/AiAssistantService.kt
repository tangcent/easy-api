 ...
 *
 * On [await] it emits an [com.itangcent.easyapi.core.ai.agent.AgentEvent.ClarificationRequested]
...
class UiClarificationGate(
    private val events: MutableSharedFlow<com.itangcent.easyapi.core.ai.agent.AgentEvent>
) : ClarificationGate {
    ...
    override suspend fun await(request: Clarification): ClarificationAnswers {
        ...
        pending = deferred
        events.emit(com.itangcent.easyapi.core.ai.agent.AgentEvent.ClarificationRequested(request))
        return deferred.await()
    }
 ...
 *
 * On [await] it emits an [com.itangcent.easyapi.core.ai.agent.AgentEvent.FileReadConsentRequested]
...
class UiFileReadConsentGate(
    private val events: MutableSharedFlow<com.itangcent.easyapi.core.ai.agent.AgentEvent>
) : com.itangcent.easyapi.core.ai.agent.FileReadConsentGate {
    ...
    override suspend fun await(requestedPath: String): Boolean {
        val deferred = CompletableDeferred<Boolean>()
        pending = deferred
        events.emit(com.itangcent.easyapi.core.ai.agent.AgentEvent.FileReadConsentRequested(requestedPath))
        return deferred.await()
    }
