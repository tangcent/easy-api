...
class BackupSettingsPanel(private val project: com.intellij.openapi.project.Project) : SettingsPanel<Settings> {
    ...
    internal fun applyImported(json: String) {
        ...
        binder.update(com.itangcent.easyapi.core.settings.module.GeneralSettings::class) {
            ...
            obj.get("searchEverywhereEnabled")?.asBoolean?.let { searchEverywhereEnabled = it }
            ...
            // values produce no entry.
            val enabled = enabledFrameworks.toMutableSet()
            val disabled = disabledFrameworks.toMutableSet()
            obj.get("feignEnable")?.takeIf { it.isJsonPrimitive }?.asBoolean?.let { legacy ->
                if (legacy) enabled.add("Feign")
            }
            obj.get("jaxrsEnable")?.takeIf { it.isJsonPrimitive }?.asBoolean?.let { legacy ->
                if (!legacy) disabled.add("JAX-RS")
            }
            obj.get("actuatorEnable")?.takeIf { it.isJsonPrimitive }?.asBoolean?.let { legacy ->
                if (legacy) enabled.add("SpringActuator")
            }
            obj.get("grpcEnable")?.takeIf { it.isJsonPrimitive }?.asBoolean?.let { legacy ->
                if (!legacy) disabled.add("gRPC")
            }
            enabledFrameworks = enabled.toTypedArray()
            disabledFrameworks = disabled.toTypedArray()
        }

        binder.update(com.itangcent.easyapi.core.settings.module.HttpSettings::class) {
        ...
        binder.update(com.itangcent.easyapi.core.settings.module.ParsingOutputSettings::class) {
        ...
        binder.update(com.itangcent.easyapi.core.settings.module.EnvironmentSettings::class) {
            obj.get("globalEnvironments")?.asString?.let { globalEnvironments = it }
        }

        binder.update(com.itangcent.easyapi.core.settings.module.RuleFileSettings::class) {
        ...
        binder.update(com.itangcent.easyapi.channel.postman.PostmanSettings::class) {
    ...
    internal fun exportSettings(): String {
        ...
        obj.addProperty("searchEverywhereEnabled", general.searchEverywhereEnabled)
        ...
        obj.add("enabledFrameworks", GsonUtils.GSON.toJsonTree(general.enabledFrameworks))
        obj.add("disabledFrameworks", GsonUtils.GSON.toJsonTree(general.disabledFrameworks))

        val http = binder.read(com.itangcent.easyapi.core.settings.module.HttpSettings::class)
        ...
        val parsingOutput = binder.read(com.itangcent.easyapi.core.settings.module.ParsingOutputSettings::class)
        ...
        val environment = binder.read(com.itangcent.easyapi.core.settings.module.EnvironmentSettings::class)
        obj.addProperty("globalEnvironments", environment.globalEnvironments)

        val ruleFile = binder.read(com.itangcent.easyapi.core.settings.module.RuleFileSettings::class)
        ...
        val postman = binder.read(com.itangcent.easyapi.channel.postman.PostmanSettings::class)
