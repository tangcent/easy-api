...
This guide walks through the most common EasyApi workflows — installation, first export, the API Explorer, search, field conversion, pre/post scripts, and AI-assisted rule creation.
...
EasyApi can export API endpoints to Postman, Markdown, cURL, IntelliJ HTTP Client, or Hoppscotch.
...
1. Configure your Hoppscotch server URL and log in via **Settings → EasyApi → Hoppscotch (Beta)**.
2. Right-click a controller class and select **EasyApi → Export to Hoppscotch**.
3. The plugin uploads the collection to your Hoppscotch instance.
...
EasyApi supports Postman-compatible **pre-request** and **post-response** scripts written in Groovy. These run before/after API calls initiated from the API Explorer.
