...
class EasyApiAssistantSkillTest : EasyApiLightCodeInsightFixtureTestCase() {
    ...
    fun testFactSheetsAreFreshFromExporter() {
        assertEquals(
            "skills/easy-api-assistant/tools.md is stale — run ./gradlew syncSkillFacts",
            SkillFactsExporter.toolsMarkdown(skillScriptsDir),
            skillDir.resolve("tools.md").readText(Charsets.UTF_8)
        )
        assertEquals(
            "skills/easy-api-assistant/locales.md is stale — run ./gradlew syncSkillFacts",
            SkillFactsExporter.localesMarkdown(),
            skillDir.resolve("locales.md").readText(Charsets.UTF_8)
        )
        assertEquals(
            "skills/easy-api-assistant/extensions.md is stale — run ./gradlew syncSkillFacts",
