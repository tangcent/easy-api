...
class AmbientPerceptionTest : EasyApiLightCodeInsightFixtureTestCase() {
    ...
    /**
     * Enables the Feign framework recognizer (`enabledFrameworks += "Feign"`)
     * and persists via [settingBinder] so `CompositeApiClassRecognizer`
     * rebuilds its cached recognizers. Always pair with [disableFeign] in a
     * `finally`. Feign is default-off, so adding it to `enabledFrameworks`
     * is the canonical enable path (resolved by `FrameworkRegistry`).
     */
    private fun enableFeign() {
        val gs = project.settings<GeneralSettings>()
        gs.enabledFrameworks = (gs.enabledFrameworks.toList() + "Feign").toTypedArray()
        gs.disabledFrameworks = gs.disabledFrameworks.filter { it != "Feign" }.toTypedArray()
        settingBinder.save(gs)
    }

    /**
     * Restores the default-off state for Feign by removing it from
     * `enabledFrameworks` (no entry needed in `disabledFrameworks` since
     * absence + `enabledByDefault=false` resolves to disabled). Guards
     * against application-scoped settings leaking across test methods.
     */
    private fun disableFeign() {
        val gs = project.settings<GeneralSettings>()
        gs.enabledFrameworks = gs.enabledFrameworks.filter { it != "Feign" }.toTypedArray()
        settingBinder.save(gs)
    }
}
